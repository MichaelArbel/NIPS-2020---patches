...
\begin{abstract}
 ...
 % \Edouard{\sout{\textbf{Louis} 
 % We present a new unsupervised visual representation method based on image patches K-nearest neighbors representation for the Mahalanobis distance \cite{chandra1936generalisedon}.
 % This method involves no reprensentation learning and achieves an accuracy on 86.2\% on CIFAR-10.
  %This suggests that the commonly used whitening step, generally characterized as a preprocessing step, is the primary driver of high performance of recent works that build more principled deep learning models for the image classification setting. 
  %Subsequently we adress the limit of such an approach on a large scale dataset such as imagenet, finding that it can obtain performance comparable to methods relying on sophisticated visual representation  learning algorithms.}}
